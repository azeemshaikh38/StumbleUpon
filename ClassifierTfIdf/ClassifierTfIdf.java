...
public class ClassifierTfIdf {
  ...
  private HashMap<String, HashMap<String, Double>> dictionaryHashMap = new HashMap<String, HashMap<String, Double>>();
	...
	for (int i=0;i<titleBodyArray.length;i++) {
	    ...
	    if (wordAndCount.length>1) {
	     if (dictionaryHashMap.get(keyValue[0]) == null) {
	      ...
	      tmpHashMap.put(wordAndCount[0], new Double(Double.parseDouble(wordAndCount[1])));
	      dictionaryHashMap.put(keyValue[0], tmpHashMap);
	     } else {
		HashMap<String, Double> tmpHashMap = dictionaryHashMap.get(keyValue[0]);
		tmpHashMap.put(wordAndCount[0], Double.parseDouble(wordAndCount[1]));
		dictionaryHashMap.put(keyValue[0], tmpHashMap);
	     }
	  }
	} 
		...
		completeBodyString = bodyString[1];
		  ...
		  while (outerMapIterator.hasNext()) {
		    ...
		    if (dictionaryHashMap.get(currentClass).containsKey(nextToken)) {
		      double termFrequency = dictionaryHashMap.get(currentClass).get(nextToken);
		      double probability = ((double)dictionaryHashMap.get(currentClass).get(nextToken))/((double)dictionaryHashMap.get(currentClass).get("TotalTfIdf"));
		      double logProbability = Math.log(probability);
		      output.collect(new Text("<TITLE>"+completeTitleString+"</TITLE><BODY>"+completeBodyString+"</BODY><CLASS>"+currentClass+"</CLASS>"), new Text(Double.toString(logProbability)));
  ...
  public static class Combine extends MapReduceBase implements Reducer<Text, Text, Text, Text> {
    public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
      ...
      output.collect(new Text(splitClass[0]+" -->"), new Text(splitClass[1]+"@"+Double.toString(sum)));
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    JobConf conf = new JobConf(ClassifierTfIdf.class);
